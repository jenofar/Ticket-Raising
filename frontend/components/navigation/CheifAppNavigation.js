...
import Feed1Navigator from './Feed1Navigator';

const Tab= createBottomTabNavigator();

const CheifAppNavigation = () => {
    return (
        // <NavigationContainer>
            <Tab.Navigator>
            <Tab.Screen name='Home' component={FeedNavigator}  options={{tabBarIcon:({size,color})=>(<MaterialCommunityIcons size={size} color={color} name="home"/>), headerShown:false }} />
                ...
                <Tab.Screen 
            name="search" 
            component={Feed1Navigator}
            options={
                ({navigation})=>({
                tabBarButton:()=><ListingsButton name='file-search' onPress={()=>navigation.navigate("search")}/>,
                tabBarIcon:({size,color})=><MaterialCommunityIcons name="search" size={size} color={color}/>})}/>
...
export default CheifAppNavigation;